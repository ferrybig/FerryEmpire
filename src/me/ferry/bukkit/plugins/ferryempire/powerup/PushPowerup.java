 ...
 * @author Fernando
 */
public class PushPowerup extends MetadataPowerup
{
	public PushPowerup()
	{
 ...
 // the location object where the smoke must form
		...
		for (Entity player1 : loc.getWorld().getEntities())
		{
			...
			if (otherLocation.distanceSquared(loc) < 49)
			{
				if (player1 != shooter)
				{
					Vector vector = new Vector(otherLocation.getX() - playerLocation.getX(), otherLocation.getY() - playerLocation.getY(),
					...
					{
						((Player) player1).sendMessage(ChatColor.DARK_RED + "You have been pushed by " + shooter.getName() + "!");
					}
				}
			}
		}
